...
const targetDate = computed(() =>
  props.target instanceof Date ? props.target : new Date(props.target),
)
...
<template>
  ...
  >
    ...
    <div
      class="rounded-3xl border border-white/30 bg-white/10 p-2 md:p-4 shadow-xl ring-1 ring-black/5 backdrop-blur-xl dark:border-white/20 dark:bg-white/5 dark:ring-white/10"
      style="
        background-image:
          radial-gradient(1200px 200px at 0% 0%, rgba(255, 255, 255, 0.35), rgba(255, 255, 255, 0)),
          radial-gradient(
            1200px 200px at 100% 100%,
    ...
    >
      ...
      <div class="text-center">
        <p v-if="title" class="text-sm uppercase tracking-[0.25em] text-rose-600 !font-bold">
          {{ title }}
        </p>
        <h2 class="mt-1 text-sm font-semibold text-gray-900 md:text-2xl">Time until the big day</h2>
        <p class="sr-only">{{ liveText }}</p>
      </div>

      <div class="mt-4 grid grid-cols-2 gap-4 md:grid-cols-4 md:mt-6">
        <div class="glass-seg">
      ...
      <div
        v-if="finished"
        class="mt-6 rounded-xl bg-emerald-50/80 p-3 text-center text-emerald-800 ring-1 ring-emerald-200"
      >
        It's time! 🎉
      </div>
    </div>
...
.glass-seg {
  @apply rounded-2xl border border-white/30 bg-white/20 p-2 md:p-4 text-center shadow-sm ring-1 ring-black/5 backdrop-blur-xl
